...
require 'funcoes.php'; // Incluído para consistência
require 'ml_api_config.php'; // Para simular a API de ML de Tendências

// Variável para marcar o link ativo na sidebar
$page_active = 'tendencias.php';

// --- Lógica de Busca de Itens Recomendados pela ML ---
$itens_recomendados = [];
try {
    // 1. Simula a chamada à API para obter IDs de itens com alta previsão de venda
    $ids_ml = simularPrevisaoML('vendas'); 
    
    if (!empty($ids_ml)) {
        // Cria placeholders seguros para a consulta
        $placeholders = implode(',', array_fill(0, count($ids_ml), '?'));
        
        // 2. Busca os detalhes dos itens recomendados
        $sql_ml = "SELECT i.id, i.nome, i.descricao, i.preco, i.quantidade, u.nome AS nome_vendedor 
                   FROM itens i 
                   JOIN usuarios u ON i.usuario_id = u.id 
                   WHERE i.id IN ({$placeholders})
                   ORDER BY FIELD(i.id, " . $placeholders . ")"; 
                   
        // Prepara os parâmetros para a cláusula IN e para a ORDER BY
        $params = array_merge($ids_ml, $ids_ml);
        
        $stmt_ml = $pdo->prepare($sql_ml);
        $stmt_ml->execute($params);
        $itens_recomendados = $stmt_ml->fetchAll(PDO::FETCH_ASSOC);
    }
} catch (\PDOException $e) {
    // Em caso de erro, apenas ignora e não exibe as recomendações
}
?>
<!DOCTYPE html>
<html lang="pt-BR">
<head>
    ...
    <title>Tendências | GreenTech Solutions</title>
    <style>
...
<body>
    ...
    <main style="text-align: center; padding-bottom: 20px;">
        <h1 style="color: #000; margin-bottom: 20px;">🔥 Tendências: Itens em Alta Recomendados</h1>
        
        <div class="items-grid">
        <?php if (!empty($itens_recomendados)): ?>
                <?php foreach ($itens_recomendados as $item): ?>
                    <div class="item-card">
            ...
            <?php else: ?>
                <p style="grid-column: 1 / -1; color: #555; font-size: 1.2em;">
                    Não foi possível carregar as tendências ou a API de ML não retornou resultados.
                </p>
            <?php endif; ?>
        </div>
    ...
    <nav class="navigation-bar">
        ...
        <a href="tendencias.php" class="nav-item active">
